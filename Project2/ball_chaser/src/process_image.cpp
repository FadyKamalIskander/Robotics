...
void process_image_callback(const sensor_msgs::Image img)
{
    ...
    for (int i = 0; i < img.height * img.step; i+=3) 
    {
           ...
           ) 
        {
            ...
            if ((stepIncrement > (img.step/6)) && (stepIncrement < (img.step*5/6)))
            {
                drive_robot(0.5, 0.0);
                isMoving = true;
            }
            else if (stepIncrement < (img.step/6))
            {
                drive_robot(0.0, 0.5);
                isMoving = true;
            }
            else //if ((stepIncrement > (img.step*2/3)) && (stepIncrement < (img.step)))
            {
                drive_robot(0.0, -0.5);
                isMoving = true;
            }
